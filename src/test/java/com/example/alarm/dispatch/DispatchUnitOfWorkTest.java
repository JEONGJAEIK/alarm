...
class DispatchUnitOfWorkTest extends AbstractMysqlIntegrationTest {
    ...
    void claimBatch는_PENDING을_IN_PROGRESS로_전이시킨다() {
        ...
        repo.save(Notification.createImmediate("u1", NotificationType.PAYMENT_CONFIRMED,
    ...
    void claimBatch는_미래의_scheduledAt을_가진_행을_제외한다() {
        Instant now = Instant.now();
        Notification due = repo.save(Notification.createImmediate("u1", NotificationType.PAYMENT_CONFIRMED,
                NotificationChannelType.EMAIL, "evt-c3", Map.of(), now.minusSeconds(5)));
        Notification later = repo.save(Notification.createScheduled("u1", NotificationType.PAYMENT_CONFIRMED,
